...
"""
def http_request_json(url, jsons=None,body=None, params="", headers={},method="POST"):
    try:
        if method == "POST":
            print_log("请求url：\n"+url)
            ...
            http_client = requests.post(url,json=jsons,data=body,headers=headers)
            print http_client.text
...
def http_request_json(url, body=None, jsons=None, params="", headers={},method="POST"):
    try:
        if method == "POST":
            print_log("请求url：\n"+url)
            if isinstance(body,basestring) and body!= None:
                body = eval(body)
            if isinstance(jsons, basestring) and jsons != None:
                jsons = eval(jsons)
            if isinstance(headers,basestring):
                headers = eval(headers)

            http_client = requests.post(url, data=body,headers=headers,json=jsons)
            print http_client.text
            if body != None:
                print body
                print_log("请求消息体：\n"+json.dumps(body,ensure_ascii=False, indent=2))
